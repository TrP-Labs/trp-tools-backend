...
export const dispatch = new Elysia({ prefix: "/dispatch", tags : ["Dispatch"]})
    ...
    .group('/:roomID', (app) => app
        .get('/', async ({ params : { roomID }, session }) => {
            ...
            const content = await DispatchControls.GetAllVehicles(roomID)
            return content
        }, {
            response : {
                200 : Vehicles.FullVehicleList,
        ...
        .get('/connect', async ({ cookie : { access_token }, params : { roomID }, session }) => {
            ...
            return sse(Stream) as unknown as Vehicles.StreamEventSchema
        })
